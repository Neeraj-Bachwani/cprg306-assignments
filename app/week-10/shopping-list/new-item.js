...
export default function NewItem({onAddItem}) {
    ...
    const handleChange = (event) => {
        ...
        let item = { id,quantity, name, category };
        onAddItem(item);
        console.log(item);
        setQuantity(1);
        setName("");
    ...
    return (
        <main className="flex w-full bg-gray-200">
            <form className="bg-electric-violet-950 w-full max-w-sm h-1/2 rounded">
                <div className="m-3">
                    <input type="text" value={name} placeholder="Item Name" className="text-gray-900 w-full p-2 rounded-md placeholder:text-gray-900" onChange={(event) => nameHandler(event)} required />
                </div>
                ...
                <div className="flex justify-between">
                    ...
                    <div className="m-3">
                        <select value={category} className="p-2 rounded-md text-gray-900 " onChange={(event) => categoryHandler(event)}>
